...
GitHub default branches ──poll──▶ bare mirrors (.cache/mirrors)
              ...
              · cross-app acceptance: real Email Watcher code → real Invoice Processor provider
              · GitHub Releases
...
Release, which none has). Linux and Windows are scored separately; Linux alone never means
ready.
...
| duplicate progress on a duplicate delivery, or lose a distinct change record | `test_duplicate_delivery_stores_once`, `test_change_records_from_different_baselines_are_both_kept` |
...
| invent data when only rendering | `test_render_only_never_invents_data` |
